...
export default function SubmitRoutineForm() {
    ...
    return (
        <>
            <Form action={formAction} className="w-[400px] mx-auto mt-10 max-h-screen overflow-hidden">
                ...
                <label htmlFor="workout-date">Routine Date:</label>
                        ...
                        {routineToSubmit.routineExercises.map((routineExercise, routineExerciseIndex) => (
                            <li key={routineExerciseIndex} id={routineExercise.id.toString()} className="bg-gray-300 w-full flex flex-col">
                                <div className="exerciseName flex flex-row justify-start items-center gap-3 py-1 px-2 text-[14px]">
                                    <p>{routineExercise.exercise?.name}</p>
                                    <TrashIcon onClick={() => handleRemoveExercise(routineExerciseIndex)} className="w-5 h-5 p-[2px] bg-gray-400 rounded-full  cursor-pointer hover:bg-gray-500"/>
                                </div>
                                    ...
                                    <ul className="setsList">
                                        {routineExercise.sets.map((set, setIndex) => (
                                            <li key={setIndex} className="flex flex-row justify-around items-center mb-2 text-[14px]">
                                                ...
                                                <Input className="w-[40px] py-[0px] pl-[5px]" type="number" min="1" name="reps" id="reps" value={set.reps} onChange={(e) => handleSetChange(routineExerciseIndex, setIndex, 'reps', Number(e.target.value))} required/>
                                                <label htmlFor="weight">Weight (kg): </label>
                                                <Input className="w-[40px] py-[0px] pl-[5px]" type="number" min="0" name="weight" id="weight" value={set.weight} onChange={(e) => handleSetChange(routineExerciseIndex, setIndex, 'weight', Number(e.target.value))}/>
                                                <MinusIcon onClick={() => handleRemoveSet(routineExerciseIndex, setIndex)} className="bg-gray-400 w-4 h-4 rounded-full cursor-pointer hover:bg-red-700" />
                                            </li>
                                        ))}
                                    </ul>
                                )}
                                <Button type="button" onClick={() => handleAddSet(routineExerciseIndex)} className="mt-[0px] mb-1 text-[14px] py-[0px] h-[30px] w-[130px] rounded-[8px]">
